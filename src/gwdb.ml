...
value un_istr2 =
  fun
  [ Istr2 db2 path pos -> string_of_istr2 db2 path pos
  | Istr2New _ s -> s ]
;

value is_empty_string istr = un_istr2 istr = "";

value is_quest_string istr = un_istr2 istr = "?";
...
value spi_find = spi2_fun.spi_find;

value spi_first = spi2_fun.spi_first;

value spi_next = spi2_fun.spi_next;
...
type person = (db2 * int * person2_dat);
...
value get_consang ((db2, i, _) as a) =
  let f pf = pf.get_consang in
  match db2.consang_array with
  [ Some tab -> tab.(i)
  | None -> wrap_asc f f a ]
;
value get_parents ((db2, i, _) as a) =
...
;
type family = (db2 * int * family2_dat);
...
value get_father ((db2, i, _) as cpl) =
  let f pf = pf.get_father in
      match db2.father_array with
      [ Some tab -> tab.(i)
      | None -> wrap_cpl f f cpl ]
;
value get_mother ((db2, i, _) as cpl) =
  ...
  self where rec self =
    {close_base () =
     ...
     persons_of_first_name () =
       (db2, (persons_of_first_name_or_surname2 db2 True));
     persons_of_surname () =
       (db2, (persons_of_first_name_or_surname2 db2 False));
...
value open_base bname =
  ...
  if Sys.file_exists (Filename.concat bname "base_d") then
    ...
    failwith "Old database format, please upgrade your database"
;
